    ...
    S.prototype.config = {
        ...
        if (a.startTime = (new Date).getTime(), n.url) {
            ...
            c[o.pageName] = e, c[o.limitName] = n.limit, t.ajax({
                ...
                //console.log(e.getResponseHeader("ajaxtimeout"));
                 if(e.getResponseHeader("ajaxtimeout")=="ajaxtimeout"){
                    window.location.href = context+"/index.jsp?tag=1"
                 }else{
                    a.layMain.html('<div class="' + f + '">网络不稳定请刷新页面</div>'), a.renderForm(), i && l.close(i)
                 }
                }
            })
        ...
        return c.key = s.id || s.index, d.cache[c.key] = u, c.layPage[0 === u.length && 1 == n ? "addClass" : "removeClass"](h), r ? v() : 0 === u.length ? (c.renderForm(), c.layFixed.remove(), c.layMain.find("tbody").html(""), c.layMain.find("." + f).remove(), c.layMain.append('<div class="' + f + '">' + s.text.none + "</div>")) : (v(), void(s.page && (s.page = t.extend({
            ...
            jump: function (e, t) {
             ...
             // $.ajax({
             //              url:context+"/checktimeout.do",
             //                  success:function(data, textStatus, jqXHR){
             //                      var ss=jqXHR.getResponseHeader("ajaxtimeout");
             //                      if("ajaxtimeout"==ss){
             //                          window.location.href = context+"/index.jsp?tag=1"
             //                      }
             //
             //                  },
             //                  error:function(e){
             //                      var ss=e.getResponseHeader("ajaxtimeout");
             //                  if("ajaxtimeout"==ss){
             //                  window.location.href = context+"/index.jsp?tag=1"
             //                  }
             //
        ...
        if(this.key == "roleList_table_id"){
            ...
            //选中状态value的装填
            $('.ckTwo').each(function (e) {
                if (this.checked == true) {
                    roleCheckedValArray.push(this.value);
                }else{
                    roleCheckedValArray.remove(this.value);
                }
                roleCheckedValArray = mainUntil.array_de_weight_fun(roleCheckedValArray);
            });
        }
        //end
