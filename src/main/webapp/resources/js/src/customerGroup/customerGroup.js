    ...
    function init() {
        ...
        layui.use(['table', 'form', 'element'], function () {
            ...
            table.render({
                ...
                , cols: [[ //标题栏
                    ...
                    ,{field: 'descri', title: '描述',align:'center', minwidth: 430}
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.status == 0) {
                                ...
                                // var node = treeObj.getNodeByParam("id", data.data.id);
                                // treeObj.selectNode(node, true);//指定选中ID的节点
                                // treeObj.expandNode(node, true, false);//指定选中ID节
                            } else {
            ...
            $("#addContengtModle").on("click", function () {
                ...
                $("#addGroupName").val('');
                $("#addGroupDescri").val('');
                var customerGroupId = ajaxParams['groupId'];
                console.log(customerGroupId);
                var node;
                if (customerGroupId != '' && customerGroupId != undefined) {
                    $("#add_contentSubjectId").val(ajaxParams['groupId']);
                    $("#add_contentSubjectName").val(ajaxParams['groupIdSName']);
                    zTree_Menu = $.fn.zTree.getZTreeObj("add_tree");
                    node = zTree_Menu.getNodeByParam("id", ajaxParams['groupId']);
                    zTree_Menu.selectNode(node, true);//指定选中ID的节点
                    zTree_Menu.expandNode(node, true, false);//指定选中ID节
                }
            ...
            $("#commitAdd").on("click", function () {
                ...
                if(textReplace.test(addGroupName.trim())){
                    layer.msg("分组标题不能含有特殊字符", {icon: 1, time: 2000});
                    return;
                }
            ...
            $("#commitUpdate").on("click", function () {
                var updGroupName=$("#updGroupName").val();
                if(textReplace.test(updGroupName.trim())){
                    layer.msg("分组标题不能含有特殊字符", {icon: 1, time: 2000});
                    return;
                }
                if (updGroupName.trim() == '') {
                    layer.msg('分组标题不能为空!', {time: 2000, icon: 1});
                    return;
                }
        ...
        function zTreeOnClick(event, treeId, treeNode) {
            ...
            if (!addFlag) { //点击添加页面的树节点
                ...
                console.log(ajaxParams['groupId']);
                ajaxParams['groupIdSName'] = name;
                table.reload('customerGroup', {
                    page: {
                        curr: 1 //重新从第 1 页开始
                    }
